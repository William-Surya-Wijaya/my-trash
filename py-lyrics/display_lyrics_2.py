...
# Function to create color changing background
def create_color_change_effect(duration, size=(1920, 1080)):
    colors = [(255, 0, 0), (0, 255, 0), (0, 0, 255)]
    clips = []
    color_duration = duration / len(colors)
    for color in colors:
        clip = ColorClip(size, color=color).set_duration(color_duration)
        clips.append(clip)
...
# Function to generate word clips
...
# Function to generate lyric clips
...
def main():
    ...
    color_change_effect = create_color_change_effect(duration)

    # Combine background effects
    background_effects = CompositeVideoClip([color_change_effect])
    ...
    final = CompositeVideoClip([background_effects, *lyric_clips])
