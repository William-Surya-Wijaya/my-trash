<?php
    ...
    if(isset($_GET['kodeTr'])){
        ...
        $query = "SELECT * FROM transparent WHERE kodeTransaksi ='".$kodeTr."'";
...
<script>
    ...
    function transRow(){
        ...
        var sendTr = "<?php echo $kodeTr; ?>";

        $.ajax({
            type : 'post',
            url : 'http://localhost/new_rumah_sakit/transactionDetail.php',
            datatype : 'text',
            data : {'sendVal' : sendTr},
            success : function(data){
                ...
                for(var i = 0; i<tempVal.length; i++){
                    ...
                    (((clone.children)[1].children)[0].value+=tempVal[i]['kodeobat']);(((((clone.children)[2].children)[0]).children)[0].value+=tempVal[i]['namaObat']);(((clone.children)[3].children)[0].value+=tempVal[i]['jumlahObat']);(((clone.children)[4].children)[0].value+=tempVal[i]['harga']);
                    ...
                    autocompleteDet(document.getElementById("medicineName"+xyzc),medVal);
                    setCurTransTotal("medicineQty"+xyzc);
                    
                    xyz += 1; document.getElementById("totalRow").value=xyz;
                }
                // $('#tempVal').val(data);
            }
        });
            ...
            for(i = 0; i<arr.length; i++){
                if(arr[i].substr(0, val.length).toUpperCase() == val.toUpperCase()){
                    ...
                    b.addEventListener("click", function(e){
                        ...
                        document.masterForm.custName.value = document.masterForm.customerName.value; closeAllLists();
                    });
                    a.appendChild(b);
                }
            }
    ...
    <div class="web-page">
        <div class="card">
            <div class="card-content">
                ...
                <div id="ha-er"></div>
                <center><form name="masterForm" id="masterForm" action="saveEditParentChild.php" method="post" autocomplete="off"><table cellspacing=0 cellpadding=0>
                    <input hidden id="totalRow" name="totalRow" value=""><input hidden id="custName" name="custName" value="<?php echo $row['namaCustomer']?>">
                    <tr>
                        <td id="form-name" style="border-top-left-radius : 8px; width : 210px;">customer</td><td><div class="autocomplete" style="width : content;"><input class="mform-answer-bar" id="customerName" nama="customerName" type="text" value="<?php echo $row['namaCustomer']?>"></div></td><td id="form-name" style="width : 210px;">kode transaksi</td>
                        <td style="border-top-right-radius : 8px;"><input class="mform-answer-bar" name="transCode" style="width : 200px; display : inline-table;" value="<?php echo $row['kodeTransaksi'] ?>" readonly></td>
                    </tr>
                    <tr>
                        <td id="form-name" style="width : 210px;">surat jalan</td><td></td><td id="form-name" style="width : 210px;">tanggal</td><td><input class="mform-answer-bar" name="transTime" style="width : 200px; display : inline-table; color : #3baaa1;" value="<?php echo $row['tglTransaksi'] ?>" readonly></td>
                    </tr>
                </table>
                <div id="ha-er"></div>
                <div class="content-wrapper" style="display : inline-block; width : 90%;">
                    <input class="float-left" type="button" onclick="addTransRowC(document.getElementById('totalRow').value);" id="true-button" value="ADD ROW"><input class="float-left" type="button" onclick="javascript:document.getElementById('masterForm').submit()" id="true-button" value="SAVE" style="margin : 5px 10px;"><input class="float-right" type="button" id="true-button" value="CANCEL">
                </div>
